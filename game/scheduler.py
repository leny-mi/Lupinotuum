...
class Scheduler:
    ...
    async def initialize(self):
        if self.time.seconds_until(19, 0) < 3600:
            ...
            await self.interface.game_broadcast(game_obj.id, "Game will start at " + self.time.get_time_in_string(1,0))
            await asyncio.sleep(3600)
        else:
            await self.interface.game_broadcast(game_obj.id, "Game will start at " + self.time.get_next_time_string(19,0))
            await self.sleep_until(19, 0)

        ## TODO: CHECK IF ALL PLAYERS JOINED

        if not utils.check_all_players_joined(self.players_list):
            await message.channel.send('Not all players have joined. Please join https://discord.gg/'+ datamanager.getConfig('invite_link') +' for private communications\nGame with ID '+str(game_obj.id)+' will start at ' + self.time.get_next_time_string(20,0))

            await self.sleep_until(20,0)
            ## TODO: If game setups at 18:59, this won't be enough time for another join

            if not utils.check_all_players_joined(self.players_list):
                await self.interface.game_broadcast(game_obj.id, "Not all players have joined. Your game has been cancelled. Use `$setup` to start a new game")


            await game_obj.commence_inital()
        ...
        await time_scheduler()

    async def time_scheduler(self):

        await game_obj.commence_night()

        while True: #Edit to end at game end
            await game_obj.sleep_until(8, 0)
            await game_obj.commence_day()
            await game_obj.sleep_until(17, 30)
            await game_obj.commence_vote()
            await game_obj.sleep_until(18, 40)
            await game_obj.commence_voteend()
            await game_obj.sleep_until(19, 0)
            await game_obj.commence_tiebreaker()
            await game_obj.sleep_until(19, 30)
            await game_obj.commence_hanging()
            await game_obj.sleep_until(20, 0)
            await game_obj.commence_night()
